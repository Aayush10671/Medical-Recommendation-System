...
# Model Prediction function - FIXED VERSION
def get_predicted_value(patient_symptoms):
    ...
    # Print warnings for invalid symptoms
    if invalid_symptoms:
        print(f"Warning: These symptoms are not recognized: {invalid_symptoms}")
    
    # Make prediction - the model returns the disease name directly
...
def home():
    if request.method == 'POST':
        ...
        print(f"User entered symptoms: {symptoms}")
        ...
        try:
            ...
            dis_des, precautions_list, medications_list, rec_diet, workout = helper(predicted_disease)
            
            # Process precautions
            my_precautions = []
            if len(precautions_list) > 0:
                for i in precautions_list[0]:
                    if i:  # Check if not empty/None
